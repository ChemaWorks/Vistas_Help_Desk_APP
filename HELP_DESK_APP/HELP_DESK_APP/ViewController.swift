...
class ViewController: UIViewController {
    ...
    @IBAction func btnIniciarSesion(_ sender: Any) {
        ...
        validarLogin(user: username, password: password) { success, user, tickets in
            DispatchQueue.main.async {
                print("Resultado de la validación: \(success)")
                if success, let user = user {
                    // Almacenar los datos del usuario en UserDefaults
                    UserDefaults.standard.set(user.idUsuario, forKey: "idUsuario")
                    ...
                    // Aquí puedes almacenar los tickets si es necesario
                    ...
                    if user.tipo == "Alumno" {
                        ...
                        if user.tipo == "Técnico"{
                            self.performSegue(withIdentifier: "TecnicoDashboard", sender: self)
                        }
                        else{
    ...
    private func validarLogin(user: String, password: String, completion: @escaping (Bool, Usuario?, [Ticket]?) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let data = data {
                do {
                    ...
                    completion(true, jsonResponse.usuario, jsonResponse.tickets)
